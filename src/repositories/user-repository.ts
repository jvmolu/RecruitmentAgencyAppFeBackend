...
class UserRepository extends BaseRepository {
    ...
    async create(user: UserType): Promise<GeneralAppResponse<User>> {
        try {
            const query: string = `INSERT INTO ${DbTable.USERS} (id, first_name, last_name, email, password, created_at, updated_at) VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING *`;
            const params: any[] = [user.id, user.firstName, user.lastName, user.email, user.password, user.createdAt, user.updatedAt];
